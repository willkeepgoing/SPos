...
from SDC import SpatialDisplacementContact
...
class Attention(nn.Module):
    def __init__(self, dim, num_patches, heads=8, dim_head=64, dropout=0., is_LT=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        dots = torch.mul(einsum('b h i d, b h j d -> b h i j', q, k),
                         self.scale.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand((b, h, 1, 1)))
...
class Transformer(nn.Module):
    def __init__(self, dim, num_patches, depth, heads, dim_head, mlp_dim_ratio, dropout=0.,
                 is_LT=False):
        super().__init__()
        ...
        for i in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(num_patches, dim,
                        Attention(dim, num_patches, heads=heads, dim_head=dim_head, dropout=dropout, is_LT=is_LT)),
